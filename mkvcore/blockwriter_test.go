...
func TestBlockWriter_WithCues(t *testing.T) {
	...
	t.Run("CuesOverflow", func(t *testing.T) {
		...
		if result.Segment.Cues == nil {
			t.Error("Expected some cues in output")
		}
		if n := len(result.Segment.Cues.CuePoint); n != 3 {
			t.Errorf("Expected 3 cue points, but have %d", n)
		}
	})
	...
	t.Run("PositionVerification", func(t *testing.T) {
		...
		for name, testCase := range testCases {
			...
			t.Run(name, func(t *testing.T) {
				...
				// Write frames, each triggering a new cluster (timecodes far apart)
